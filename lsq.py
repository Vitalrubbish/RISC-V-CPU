...
class LSQ(Module):
    ...
    def build(
        ...
        head = RegArray(Int(5), 1, initializer=[0])          # 存储 LSQ 的头指针
        tail = RegArray(Int(5), 1, initializer=[0])          # 存储 LSQ 的尾指针
        lsq_size = RegArray(Int(5), 1)                       # 存储 LSQ 目前指令的条数
        ...
        updated_tail_ptr = tail_ptr + Int(5)(1)
        updated_tail_ptr = (updated_tail_ptr == Int(5)(LSQ_SIZE)).select(Int(5)(0), updated_tail_ptr)
        updated_lsq_size = lsq_size[0] + Int(5)(1)

        lsq_full = (updated_lsq_size == Int(5)(LSQ_SIZE)).select(Bits(1)(1), Bits(1)(0))

        write_valid = lsq_write & ~lsq_full
        with Condition(lsq_write & ~lsq_full):
            log("LSQ entry {} allocated", tail_ptr)
            write1hot(allocated_array, tail_ptr, Bits(1)(1))
            rob_index_array[tail_ptr] = rob_index
            is_load_array[tail_ptr] = signals.memory[0:0]
            is_store_array[tail_ptr] = signals.memory[1:1]
            imm_array[tail_ptr] = signals.imm
            rs1_array[tail_ptr] = signals.rs1
            write1hot(rs1_value_array, tail_ptr, rs1_value)
            has_rs1_array[tail_ptr] = signals.rs1_valid
            rs1_recorder_array[tail_ptr] = rs1_recorder
            write1hot(has_rs1_recorder_array, tail_ptr, rs1_has_recorder)
            rs2_array[tail_ptr] = signals.rs2
            write1hot(rs2_value_array, tail_ptr, rs2_value)
            has_rs2_array[tail_ptr] = signals.rs2_valid
            rs2_recorder_array[tail_ptr] = rs2_recorder
            write1hot(has_rs2_recorder_array, tail_ptr, rs2_has_recorder)
            
            write1hot(ready_array, tail_ptr, ~((signals.rs1_valid & rs1_has_recorder) | (signals.rs2_valid & rs2_has_recorder)))
        ...
        alu_a = read_mux(rs1_value_array, head_ptr, LSQ_SIZE, 32)
        alu_b = imm_array[head_ptr]
        alu_result = (alu_a.bitcast(Int(32)) + alu_b.bitcast(Int(32))).bitcast(Bits(32))

        is_memory_read = is_load_array[head_ptr]
        is_memory_write = is_store_array[head_ptr]
        ...
        dcache_wdata = read_mux(rs2_value_array, head_ptr, LSQ_SIZE, 32)

        execute_valid = read_mux(allocated_array, head_ptr, LSQ_SIZE, 1) & read_mux(ready_array, head_ptr, LSQ_SIZE, 1) & (~clear_signal_array[0])
        with Condition(execute_valid):
            # 执行 head 指向的条目
            log("LSQ entry {} executed", head_ptr)

            write1hot(allocated_array, head_ptr, Bits(1)(0))
            head[0] = (head_ptr + Int(5)(1) == Int(5)(LSQ_SIZE)).select(Int(5)(0), head_ptr + Int(5)(1))
        ...
        with Condition(lsq_modify_recorder):
            for i in range(LSQ_SIZE):
                ...
                with Condition(~(write_valid & (tail_ptr == Int(5)(i)))):
                    write1hot(ready_array, Bits(5)(i), ~((has_rs1_array[i] & (has_rs1_recorder_array[i][0] & (~modify_rs1_recorder))) | 
        ...
        with Condition(clear_signal_array[0]):
            ...
            lsq_size[0] = Int(5)(0)
            for i in range(LSQ_SIZE):
                write1hot(allocated_array, Bits(5)(i), Bits(1)(0))

        rob_index_array_ret[0] = rob_index_array[head_ptr]
        pc_result_array[0] = (addr_array[head_ptr].bitcast(Int(32)) + Int(32)(4)).bitcast(Bits(32))
        is_head_ready = read_mux(allocated_array, head_ptr, LSQ_SIZE, 1) & read_mux(ready_array, head_ptr, LSQ_SIZE, 1)
        signal_array[0] = is_head_ready.select(Bits(1)(1), Bits(1)(0))
        
        with Condition(~clear_signal_array[0]):
            lsq_size[0] = lsq_size[0] + write_valid.select(Int(5)(1), Int(5)(0)) - execute_valid.select(Int(5)(1), Int(5)(0))