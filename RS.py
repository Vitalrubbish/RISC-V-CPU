...
class RS(Module):
    ...
    def build(
        ...
        with Condition(rs_write & ~allocated):
            ...
            write1hot(rs1_value_array, rob_index, rs1_value)
            rs2_array[rob_index] = signals.rs2
            has_rs2_array[rob_index] = signals.rs2_valid
            write1hot(rs2_value_array, rob_index, rs2_value)
            rs1_recorder_array[rob_index] = rs1_recorder
            write1hot(has_rs1_recorder_array, rob_index, rs1_has_recorder)
            rs2_recorder_array[rob_index] = rs2_recorder
            write1hot(has_rs2_recorder_array, rob_index, rs2_has_recorder)
        ...
        for i in range(RS_SIZE):
            allocated = read_mux(allocated_array, Bits(5)(i), RS_SIZE, 1).select(Bits(1)(1), Bits(1)(0))
            ...
            send_index = valid.select(Bits(5)(i), send_index)
        ...
        with Condition(rs_modify_recorder):
            ...
            for i in range(RS_SIZE):
                with Condition(read_mux(allocated_array, Bits(5)(i), RS_SIZE, 1) & (rs1_recorder_array[i] == rs_recorder)):
                ...
                with Condition(read_mux(allocated_array, Bits(5)(i), RS_SIZE, 1) & (rs2_recorder_array[i] == rs_recorder)):
